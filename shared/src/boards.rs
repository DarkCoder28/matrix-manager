...
impl Default for BoardDefinition {
    fn default() -> Self {
        BoardDefinition {
            ...
            size: (64, 32),
            board_elements: vec![
                BoardElementBuilder::default().name(String::from("Weekday")).y(0).value(BoardElementValue::Text(String::from("__weekday__"))).build().unwrap(),
                BoardElementBuilder::default().name(String::from("Time")).y(9).font(Some(String::from("7x14B"))).value(BoardElementValue::Text(String::from("__time__"))).build().unwrap(),
                BoardElementBuilder::default().name(String::from("Date")).y(24).value(BoardElementValue::Text(String::from("__date__"))).build().unwrap(),
            ],
        }
    }
}

#[derive(Serialize, Deserialize, Builder, Clone, PartialOrd)]
#[builder(default)]
pub struct BoardElement {
    pub name: String,
    pub x: Option<u8>,
    pub y: u8,
    pub colour: Option<ElementColour>,
    pub font: Option<String>,
    pub value: BoardElementValue,
}
impl PartialEq for BoardElement {
    fn eq(&self, other: &Self) -> bool {
        self.name == other.name && self.x == other.x && self.y == other.y && self.colour == other.colour && self.font == other.font && self.value == other.value
    }
...
impl Default for ElementColour {
    fn default() -> Self {
        ElementColour { r: 0xFF, g: 0xFF, b: 0xFF, a: 0xFF }
    }
}
